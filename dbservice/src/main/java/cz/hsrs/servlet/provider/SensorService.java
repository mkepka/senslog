	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			if (request.getParameter(ServiceParameters.OPERATION).equals(
					GET_SENSORS)) {
				DBJsonUtils.writeJSON(out, db.sensorUtil.getUnitsSensors(params
						.getUnit_id()));
			} else if (request.getParameter(ServiceParameters.OPERATION)
					.equals(GET_OBSERVATIONS)) {
				/**
				 * Pokud je dan trunc
				 */
				if (params.trunc != null) {

					DBJsonUtils.writeJSON(out, db.sensorUtil
							.getSensorObservationsTrunc(params.getUnit_id(),
									params.getSensor_id(), params.from,
									params.to, params.trunc));
				} else {
					DBJsonUtils.writeJSON(out, db.sensorUtil
							.getSensorObservations(params.getUnit_id(), params
									.getSensor_id(), params.from, params.to));
				}
